...
void s_cache_http_response_handler(JNIEnv *env) {
    ...
    s_jni_http_callback_handler.onResponseBody = (*env)->GetMethodID(env, cls, "onResponseBody", "(Ljava/nio/ByteBuffer;)V");
    assert(s_jni_http_callback_handler.onResponseBody);

    s_jni_http_callback_handler.onResponseComplete = (*env)->GetMethodID(env, cls, "onResponseComplete", "(I)V");
